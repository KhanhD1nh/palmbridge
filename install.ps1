# Hands — native Windows install (port of install.sh)
# Builds hands.exe from xai-org/grok-build source, downloads tunnel-client.exe.
# Requires: git, python3 (py), rustup (stable-msvc), VS Build Tools (C++).
param(
    [string]$RepoRoot = (Split-Path -Parent $MyInvocation.MyCommand.Path),
    [string]$Prefix = "$env:USERPROFILE\.local",
    [string]$Cache = "$env:USERPROFILE\.cache\hands"
)
...
Copy-Item "$GROK_BUILD\target\release\hands.exe" "$Prefix\bin\hands.exe" -Force
...
& "$Prefix\bin\hands.exe" --version
Write-Host ""
Write-Host "installed $Prefix\bin\hands.exe"
Write-Host "next: cd /your/repo && hands setup"
